...
WS1_DISPLAY_CALL_NUMBER = column_index_from_string('D')
WS1_BARCODE = column_index_from_string('BE')
WS1_ISBN = column_index_from_string('BD')
WS1_PUBLICATION_YEAR = column_index_from_string('O')
WS1_TITLE = column_index_from_string('F')
...
WS2_ISBN = column_index_from_string('AA')

# Load the workbooks
wb1 = load_workbook(Path('input', 'hsl_original.xlsx'))
wb2 = load_workbook(Path('input', 'hsl_reviewed.xlsx'))

# Target the desired worksheets
ws1 = wb1['All HSL Print Books']
...
def main():
    ...
    filename = Path('output', f'HSL {current_datetime}.xlsx')
